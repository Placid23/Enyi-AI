...
import { Loader2, Images, Trash2, AlertTriangle, Download } from 'lucide-react';
...
export default function ImageHistoryPage() {
  ...
  const handleDownloadPng = (imageDataUri: string, promptText: string) => {
    const filename = sanitizeFilename(promptText || 'history_image');
    downloadDataUri(imageDataUri, `${filename}.png`);
  };

  const handleDownloadJpeg = async (imageDataUri: string, promptText: string) => {
    const filename = sanitizeFilename(promptText || 'history_image');
    try {
      await convertToJpegAndDownload(imageDataUri, filename, 0.85);
    } catch (e) {
  ...
  return (
    <div className="flex flex-col h-full w-full p-4 md:p-6">
      <Card className="flex-grow flex flex-col shadow-xl rounded-xl overflow-hidden border-border/30 bg-card">
                       ...
                       <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="sm" className="w-full text-xs">
                            <Download className="mr-2 h-3.5 w-3.5" /> Download Options
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-56">
                          <DropdownMenuItem onClick={() => handleDownloadPng(entry.imageDataUri, entry.prompt)}>
                            Download PNG (Original)
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={() => handleDownloadJpeg(entry.imageDataUri, entry.prompt)}>
